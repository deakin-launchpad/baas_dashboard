    ...
    return (
      <Table>
        <TableHead>
          <TableRow>
            {_keys.map((value) => {
              return <TableCell key={Math.random()}>{value}</TableCell>;
            })}
          </TableRow>
        </TableHead>
        <TableBody>
                  ...
                  return (
                    <TableCell key={Math.random()}>
                      <Typography varient="body1">
                        {Array.isArray(value[valueA])
                          ? breakObject(value[valueA])
                          : String(value[valueA])}
                      </Typography>
                    </TableCell>
                  );
    ...
    >
      {props.sort ? (
        <TableSortLabel
          ...
          onClick={(e) =>
            props.onSortChange instanceof Function &&
            props.onSortChange(e, props.value)
          }
        >
    ...
    return props.options.actions.map((value, i) => {
      ...
      return (
        <Heading
          ...
          value={
            props?.options?.titleCaseHeadings
              ? TextHelper.titleCase(value.name)
              : value.name
          }
        />
      );
    ...
    <TableHead>
      <TableRow>
        {props?.options?.selector &&
          (props.options.toolbarActions !== undefined ? (
            <Heading key={Math.random()} value={"selection"} />
          ) : null)}
...
const TableErrorAndEmptyRows = (props) => {
  ...
  if (
    props.emptyRows() > 0 &&
    !(props.options && props.options.ui && props.options.ui.disableContainer)
  )
    return (
      ...
      <TableCell
        style={
          props.styles !== undefined
            ? props.styles.tableCell !== undefined
...
const TableRows = (props) => {
  let prepareCurrentRowData = useCallback(() => {
    ...
    if (props.sort?.sort) {
      _data.sort((a, b) => {
        if (
          a[props.sort.selectedSortVariable] ===
          b[props.sort.selectedSortVariable]
        )
          return 0;
    ...
    (__obj) => {
      ...
      if (props?.options?.actions !== undefined) {
        return props.options.actions.map((value) => {
          if (value === false) return null;
          defaultValue =
            value.defaultValueFrom !== undefined
            ...
            <TableCell key={Math.random()}>
              {value.type === "switch" ? (
                <ActionButtonSwitch
                  ...
                  function={(e) => value.function(e, __obj)}
                />
              ) : (
                <ActionButton
                  key={Math.random()}
    ...
    return (
      <TableRow key={`TableRow_${index}`}>
        ...
        {props.options !== undefined
          ? props.options.actionLocation === "start"
            ? props.options.actions !== undefined
              ? renderActions(currentRowData[index])
              : null
            : null
          : null}
        <RenderRow
          key={`TableRowData_${index}`}
          page={props.page}
          data={row}
          keys={props.keys}
        />
...
const TablePaginationComponent = (props) => {
  const TablePaginationActions = () => {
    ...
    const handleFirstPageButtonClick = () => {
      if (props.options === undefined) return props.setPage(0);
      if (!props.options.disablePaginationDefaults) props.setPage(0);
      if (props?.options?.pagination?.onFirstButtonClick instanceof Function)
        props.options.pagination.onFirstButtonClick();
    };
    const handleBackButtonClick = () => {
      if (props.options === undefined) return props.setPage((page) => page - 1);
    ...
    const handleLastPageButtonClick = () => {
      if (props.options === undefined)
        return props.setPage(
          Math.max(0, Math.ceil(props.data.length / props.rowsPerPage) - 1)
        );
      if (!props.options.disablePaginationDefaults)
        props.setPage(
          Math.max(0, Math.ceil(props.data.length / props.rowsPerPage) - 1)
...
export const EnhancedTable = (props) => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(
    props?.options?.rowsPerPage ? props.options.rowsPerPage : 5
  );
  ...
  return (
    ...
    >
      <TableHeader
        {...props}
        keys={keys}
        disable={props.options?.ui?.disableTitle || false}
